...
describe("hooks - Slack E2E simulation", () => {
  test("fetch intercept strips Slack mailto markup and obfuscates PII", async () => {
    ...
    const slackMessage = 'fomat as json please: <mailto:walter@example.test|walter@example.test>';
    ...
    expect(result.obfuscated).not.toContain("walter@example.test");
  });
  ...
  test("fetch intercept re-obfuscates assistant message with Slack content blocks", async () => {
    ...
    const r1 = obf.obfuscate("walter@example.test");
    const fake = r1.mappingsUsed["walter@example.test"];
    ...
    const assistantBlock = { type: "text", text: `{"email": "walter@example.test"}` };
    ...
    expect(r2.obfuscated).not.toContain("walter@example.test");
  ...
  test("message_sending deobfuscates channel output with fake email", async () => {
    ...
    const r = obf.obfuscate("walter@example.test");
    const fake = r.mappingsUsed["walter@example.test"];
    ...
    expect(result?.content || `{"email": "${fake}"}`).toContain("walter@example.test");
  });
  ...
  test("full Slack flow: obfuscate→LLM→deobfuscate→channel — single output, no fakes", async () => {
    ...
    const slackInput = 'format as json: <mailto:walter@example.test|walter@example.test>';
    ...
    expect(obfuscatedPrompt).not.toContain("walter@example.test");

    // Step 3: LLM receives obfuscated prompt, responds with the fake
    const fake = obf.obfuscate("walter@example.test").mappingsUsed["walter@example.test"];
    ...
    expect(deobResponse).toContain("walter@example.test");
    ...
    expect(finalContent).toContain("walter@example.test");
    ...
    expect(outputEmails).toEqual(["walter@example.test"]);
  });
  ...
  test("multi-turn: second turn fetch intercept prevents PII leak from assistant history", async () => {
    ...
    // Turn 1
    const t1Result = await handlers["before_prompt_build"]({ prompt: "format: walter@example.test" });
    const fake = obf.obfuscate("walter@example.test").mappingsUsed["walter@example.test"];
    ...
    const turn2Messages = [
      ...
      { role: "assistant", content: [{ type: "text", text: `{"email": "walter@example.test"}` }] },
    ...
    expect(assistantText).not.toContain("walter@example.test");
