...
describe("file_path_unix: operational paths must NOT be obfuscated", () => {
  ...
  test("OpenClaw workspace paths pass through", () => {
    const input = "Upload the file from /home/ka/.openclaw/media/report.pdf to Slack";
    const result = ob.obfuscate(input);
    expect(result.obfuscated).toContain("/home/ka/.openclaw/media/report.pdf");
  });

  test("OpenClaw extension paths pass through", () => {
    const input = "Plugin installed at /home/ka/.openclaw/extensions/shroud-privacy/dist/index.js";
    const result = ob.obfuscate(input);
    expect(result.obfuscated).toContain("/home/ka/.openclaw/extensions/shroud-privacy/dist/index.js");
  });

  test("Script paths in exec commands pass through", () => {
    const input = "python3 /home/ka/.openclaw/workspace/scripts/searxng_search.py \"semiconductor\"";
    const result = ob.obfuscate(input);
    expect(result.obfuscated).toContain("/home/ka/.openclaw/workspace/scripts/searxng_search.py");
  });
  ...
  test("node_modules paths under /home pass through", () => {
    const input = "Module at /home/ka/.npm-global/lib/node_modules/openclaw/node_modules/@mariozechner/pi-ai/dist/utils/event-stream.js";
    const result = ob.obfuscate(input);
...
Segment breakdown:
  ...
  test("OpenClaw agent exec output does not trigger mass file_path detections", () => {
    const input = `Running: python3 /home/ka/.openclaw/workspace/scripts/searxng_search.py "ASML earnings"
Output saved to /tmp/search-results-abc123.json
Reading /home/ka/.openclaw/workspace/memory/2026-03-28.md
Writing /home/ka/.openclaw/semiconalpha-workspace/drafts/asml-q4.md
Converting: python3 /home/ka/.openclaw/semiconalpha-workspace/scripts/md_to_pdf.py drafts/asml-q4.md drafts/asml-q4.pdf
Copying to /home/ka/.openclaw/media/asml-q4.pdf
Uploading via: openclaw message send --channel slack --target C0AN09SPT29 --message "ASML Q4 draft" --media /home/ka/.openclaw/media/asml-q4.pdf`;
    const result = ob.obfuscate(input);
    const fpEntities = result.entities.filter(e => e.category === "file_path");
    expect(fpEntities).toHaveLength(0);
  });
  ...
  test("mixed conversation with PII + workspace paths: only PII obfuscated", () => {
    const input = `Walter asked me to check admin@acme.com's access to 10.1.0.1.
Script: /home/ka/.openclaw/workspace/scripts/audit_access.py
...
Found 3 login attempts from 192.168.1.100.`;
    ...
    expect(result.obfuscated).toContain("/home/ka/.openclaw/workspace/scripts/audit_access.py");
...
describe("isDocExample: file_path category", () => {
  test("home directory paths are skipped", () => {
    expect(isDocExample("/home/ka/.openclaw/media/file.pdf", "file_path")).toBe(true);
  });
