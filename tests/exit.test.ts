...
describe("EXIT 1: Bare constructor without resolveConfig", () => {
  test("phone numbers are replaced, not dropped", () => {
    ...
    const result = ob.obfuscate("Call +15550101000");
    expect(result.obfuscated).not.toContain("+15550101000");
    const fake = result.mappingsUsed["+15550101000"];
  ...
  test("emails (real domain) are replaced, not dropped", () => {
    const ob = bareObfuscator();
    const result = ob.obfuscate("mail user@example.test");
  ...
  test("multiple entities in one pass — none dropped", () => {
    ...
    const result = ob.obfuscate(
      "From: +15550101000, IP: 10.20.30.40, email: admin@corp.net"
    );
  ...
  test("full OpenClaw reproduction scenario", () => {
    ...
    const input = [
      "From: User (+15550101000)",
      "Gateway phone: +15550101001",
    ...
    expect(result.obfuscated).not.toContain("+15550101000");
    expect(result.obfuscated).not.toContain("+15550101001");
    expect(result.obfuscated).toContain(result.mappingsUsed["+15550101000"]);
    expect(result.obfuscated).toContain(result.mappingsUsed["+15550101001"]);
    ...
    // Roundtrip
    const restored = ob.deobfuscate(result.obfuscated);
    expect(restored).toContain("+15550101000");
    expect(restored).toContain("+15550101001");
  });
});
...
describe("EXIT 4: Per-category detection and roundtrip", () => {
  ...
  const cases: Array<{ name: string; input: string; realValue: string; category: string }> = [
    ...
    { name: "api_key_sk", input: "Key SHROUD_TEST_API_KEY", realValue: "SHROUD_TEST_API_KEY", category: "api_key" },
...
describe("EXIT 22: API keys and cloud tokens", () => {
  test("generic sk- key", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("key: SHROUD_TEST_API_KEY");
    expect(r.obfuscated).not.toContain("SHROUD_TEST_API_KEY");
  });
  ...
  test("AWS access key (AKIA)", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("SHROUD_TEST_AWS_ACCESS_KEY");
    expect(r.obfuscated).not.toContain("SHROUD_TEST_AWS_ACCESS_KEY");
  });
  test("AWS secret key", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("aws_secret_access_key=SHROUD_TEST_AWS_SECRET_KEY");
    expect(r.obfuscated).not.toContain("SHROUD_TEST_AWS_SECRET_KEY");
  });
  test("GCP API key (AIza)", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("SHROUD_TEST_GOOGLE_API_KEY");
    expect(r.obfuscated).not.toContain("SHROUD_TEST_GOOGLE_API_KEY");
  });
  ...
  test("GitHub PAT (ghp_)", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("SHROUD_TEST_GITHUB_TOKEN");
    expect(r.obfuscated).not.toContain("SHROUD_TEST_GITHUB_TOKEN");
  });
...
describe("EXIT 23: Network infrastructure", () => {
  ...
  test("URL with query password param", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("https://app.com/login?password=SHROUD_TEST_PASSWORD");
...
describe("EXIT 25: Format preservation", () => {
  ...
  test("fake sk- key preserves prefix", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate("SHROUD_TEST_API_KEY");
    const fake = r.mappingsUsed["SHROUD_TEST_API_KEY"];
...
describe("EXIT 32: Cross-category interactions", () => {
  ...
  test("multiple categories in single line", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate(
      "User admin@corp.com from 10.0.0.1 called +14155551234 with key SHROUD_TEST_API_KEY"
    );
...
describe("EXIT 36: Bare constructor — no category drops replacements", () => {
  const inputs: Array<{ name: string; input: string; real: string }> = [
    ...
    { name: "api key", input: "SHROUD_TEST_API_KEY", real: "SHROUD_TEST_API_KEY" },
...
describe("EXIT 37: Roundtrip matrix", () => {
  const cases: Array<{ name: string; input: string }> = [
    ...
    { name: "phone only", input: "+15550101003" },
...
describe("EXIT 43: Connection strings", () => {
  ...
  const queryParams: Array<{ name: string; input: string; secret: string }> = [
    { name: "password", input: "https://app.com?password=SHROUD_TEST_PASSWORD", secret: "MyP@ss123" },
    { name: "secret", input: "https://app.com?secret=TopS3cret", secret: "TopS3cret" },
    { name: "token", input: "https://app.com?token=SHROUD_TEST_TOKEN", secret: "abc123def456" },
...
`);
  ...
  test("mixed PII email thread", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate(`
From: User Example <user@example.test>
...
+43 664 8563582
`);
    expect(r.obfuscated).not.toContain("user@example.test");
  ...
  test("cloud credentials block", () => {
    const ob = resolvedObfuscator();
    const r = ob.obfuscate(`
AWS_ACCESS_KEY_ID=SHROUD_TEST_AWS_ACCESS_KEY
...
API_KEY=SHROUD_TEST_OPENAI_KEY
`);
    expect(r.obfuscated).not.toContain("SHROUD_TEST_AWS_ACCESS_KEY");
    ...
    expect(r.obfuscated).not.toContain("SHROUD_TEST_OPENAI_KEY");
  });
